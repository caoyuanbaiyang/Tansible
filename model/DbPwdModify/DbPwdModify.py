...
class ModelClass(object):
    ...
    def __download_modify_or_upload_cfgfile_inner(self, sftp, hostname, cfg_item, ctype):
            remote_file = cfg_item["cfg_file"]
            (tmppath, filename) = os.path.split(remote_file)
            if not os.path.exists(os.path.join(down_dir, hostname)):
                os.makedirs(os.path.join(down_dir, hostname))
            # 下载文件名称
            local_file = os.path.join(down_dir, hostname, filename)
            # 备份文件名称
            ...
            if ctype == "DOWNLOAD":
                try:
                    ...
                    raise Exception("文件下载失败{host}:{file}".format(host=hostname, file=remote_file))
    ...
    def action(self, ssh, hostname, param, hostparam=None):
        ...
        for task in param["action"]:
            if task in ["DOWNLOAD", "UPLOAD", "ROLLBACK"]:
                ...
                self.__download_modify_or_upload_cfgfile_inner(sftp, hostname, param, task)
            else:
